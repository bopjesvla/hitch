...
for (lat, lon), g in stacked_grid.items():
    if g == g:
        if VAR == 'distance' and DIVIDER == 'wait':
            ...
            val = cm.RdYlGn(.9 - 0.9 * min(g, 120) / 120)
            tooltip = f'{int(g)} min'
        elif VAR == 'distance':
            val = cm.RdYlGn(0.9 * min(g, 120) / 120)
            tooltip = f'{int(g)} km'
        c = colors.rgb2hex(val)
        folium.Rectangle(bounds=[[lat.left, lon.left], [lat.right, lon.right]], stroke=False, fill=True, fill_color=c, fill_opacity=0.3, tooltip=tooltip).add_to(m)
